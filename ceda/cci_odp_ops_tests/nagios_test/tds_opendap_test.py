...
import ceda.cci_odp_ops_tests.test_tds_opendap

log = logging.getLogger(__name__)


class TdsOpendapNagiosCtx(nagiosplugin.context.Context):
    ...
    def evaluate(self, metric, resource):
        ...
        tests = unittest.defaultTestLoader.loadTestsFromName(test_name,
                                module=ceda.cci_odp_ops_tests.test_tds_opendap)
...
class TdsOpendapResource(nagiosplugin.Resource):
    '''Nagios resource abstraction - CCI TDS OPeNDAP in this case
    '''
    def __init__(self, test_names):
        '''Overload to pass special test_names parameter'''
        super(TdsOpendapResource, self).__init__()

        if test_names is None:
            self.test_names = ['TdsOpendapTestCase']
        else:
    ...
    def probe(self):
        ...
        for test_name in self.test_names:
            ...
            yield nagiosplugin.Metric(test_name, True, context='TdsOpendapCtx')


class TdsOpendapTestResultsSummary(nagiosplugin.Summary):
...
def main():
    ...
    if '-h' in sys.argv:
        ...
        test_names = ['TdsOpendapTestCase.{}'.format(name_)
                      for name_ in dir(
                    ceda.cci_odp_ops_tests.test_tds_opendap.TdsOpendapTestCase)
                      if name_.startswith('test')]
        test_names_displ = '-h|TdsOpendapTestCase|' + '|'.join(test_names)
    ...
    check = nagiosplugin.Check(TdsOpendapResource(test_names),
                               TdsOpendapNagiosCtx('TdsOpendapCtx'),
                               TdsOpendapTestResultsSummary())
    check.name = 'TDS-OPENDAP'
    check.main()
